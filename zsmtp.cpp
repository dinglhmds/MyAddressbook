...
void ZSmtp::getMessage()
{
    ...
    else if(status==4 && serverText.indexOf("354")!=-1)
    {
        ...
        text.append(QByteArray().append(title.toStdString().data()).toBase64());
        text.append("?=\r\n");
        text.append("To: ");
        text.append(to.toStdString().data());
        text.append("\r\nFrom: ");
        text.append(username.toStdString().data());
        text.append(" <");
        text.append(username.toStdString().data());
        text.append(">\r\n");
        ...
        QString t = this->text;
        t.replace("\n", "\r\n").replace("\r\n.\r\n", "\r\n..\r\n");
        text.append(t.toStdString().data());
        text.append("\r\n.\r\n");
        status = 5;
    }
    ...
    {
        //登录
        text.append(QByteArray().append(username.toStdString().data()).toBase64());
        text.append("\r\n");
        text.append(QByteArray().append(password.toStdString().data()).toBase64());
        text.append("\r\n");
        status = 2;
    }
    else if(status==0 && serverText.indexOf("220")!=-1)
    {
        //打招呼
        text.append("EHLO ");
        text.append(serverIP.toStdString().data());
        text.append("\r\nAUTH LOGIN\r\n");
        status = 1;
    }
